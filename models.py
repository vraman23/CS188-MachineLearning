...
class RegressionModel(object):
    ...
    def run(self, x):
        ...
        rW2 = nn.Linear(nn.ReLU(nn.AddBias(xW1, self.b1)), self.W2)
        return nn.AddBias(rW2, self.b2)
    ...
    def train(self, dataset):
        ...
        alpha = 0.01
        while(loss > 0.02):
            grad_loss = self.get_loss(nn.Constant(dataset.x), nn.Constant(dataset.y))
            loss = nn.as_scalar(grad_loss)
            grad = nn.gradients(grad_loss, [self.W1, self.b1, self.W2, self.b2])
    ...
    def train(self, dataset):
        ...
        while(dataset.get_validation_accuracy() < 0.975):
            ...
            for x, y in dataset.iterate_once(60):
                grad_loss = self.get_loss(x, y)
                grad = nn.gradients(grad_loss, [self.W1, self.b1, self.W2, self.b2])
                self.W1.update(grad[0], -alpha)
                self.b1.update(grad[1], -alpha)
                self.W2.update(grad[2], -alpha)
                self.b2.update(grad[3], -alpha)
                # print(dataset.get_validation_accuracy(), nn.as_scalar(grad_loss))
...
class LanguageIDModel(object):
    ...
    def run(self, xs):
        ...
        for i in range(len(xs)):
            if i == 0:
                Z_i = nn.Linear(xs[0], self.W)
                H_i = nn.ReLU(nn.AddBias(Z_i, self.b))
            else:
                Z_i = nn.Add(nn.Linear(xs[i], self.W), nn.Linear(H_i, self.W_hidden))
                H_i = nn.ReLU(nn.AddBias(Z_i, self.b_hidden))
        return nn.AddBias(nn.Linear(H_i, self.W_end), self.b_end)
    ...
    def train(self, dataset):
        ...
        while(dataset.get_validation_accuracy() < 0.85):
            ...
            for xs, y in dataset.iterate_once(100):
                ...
                grad = nn.gradients(grad_loss, [self.W, self.b, self.W_hidden, self.b_hidden, self.W_end, self.b_end])
